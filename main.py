...
with st.sidebar:
    st.image("https://raw.githubusercontent.com/pytorch/hub/master/images/dog.jpg", caption="Sample Image")
    st.write("This app uses a pretrained ResNet18 model to visualize image predictions and explanations.")
...
selected_layer = st.sidebar.selectbox(
    "Layer",
    available_layers,
    index=available_layers.index("layer1")
)
    ...
    with st.spinner("Processing..."):
        ...
        with st.sidebar:
            ...
            for i in range(top5_prob.size(0)):
                ...
                st.write(f"{i + 1}. {top5_catid[i].item()} - {str(class_name)} - {top5_prob[i].item():.4f}")
        ...
        try:
            ...
            with col2:
                st.write("Using `SmoothGradCAMpp` for Class Activation Map (CAM) extraction.")
                st.image(cam_image, caption="Class Activation Map", use_container_width=True)
        ...
        try:
            ...
            attributions_ig_image = Image.fromarray((attributions_ig * 255).astype('uint8'))    
            with col1:
                st.write("Using `Integrated Gradients` for SHAP values.")
                st.image(attributions_ig_image, caption="SHAP Values using Integrated Gradients", use_container_width=True)
        except Exception as e:
